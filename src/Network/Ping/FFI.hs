...
foreign import ccall unsafe "oping.h ping_construct"
...
foreign import ccall unsafe "oping.h ping_get_error"
  ping_get_error :: PingObj -> IO CString

foreign import ccall unsafe "oping.h ping_iterator_get_info"
...
test = do
  ...
  dah <- ping_host_add po h
  ping_send po
  pit <- ping_iterator_get po
  latPtr <- mallocBytes 8
  latSize <- mallocBytes 8
  poke (castPtr latSize) (CSize 8)
  dgi <- ping_iterator_get_info pit (packInfo INFO_LATENCY) latPtr (castPtr latSize)
  lat <- (peek (castPtr latPtr) :: IO Double)
  print lat
  free latPtr
  free latSize
